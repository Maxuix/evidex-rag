...
export function ActivitySourcePreview({ client, run, source, trigger, onClose }: {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        const scope = run.knowledge_bases?.find(item => item.knowledge_base_id === document.kb_id);
        const revision = scope?.index_revision_id ?? (run.knowledge_base_id === document.kb_id ? run.index_revision_id : null);
        if ((source.knowledge_base_id && source.knowledge_base_id !== document.kb_id) || (source.index_revision_id && source.index_revision_id !== revision) || document.deleted_at || document.current_version?.id !== source.document_version_id || !revision || document.index?.index_revision_id !== revision) throw new Error("这份资料的原版本已不可用，不能用当前版本替代历史来源。");
        let cursor: string | undefined;
        const seen = new Set<string>();
        do {
          const page = await client.getDocumentChunks(source.document_id, cursor);
          if (cancelled) return;
          if (page.document_id !== source.document_id || page.document_version_id !== source.document_version_id || page.index_revision_id !== revision) throw new Error("资料版本已变化，无法读取这次调用的原片段。");
  ...
  }, [client, run.run_id, run.knowledge_base_id, run.index_revision_id, run.knowledge_bases, source, trigger, onClose]);
  return <><button className="drawer-backdrop" aria-label="关闭检索资料" onClick={onClose} /><aside ref={panel} className="evidence-drawer activity-source-drawer" role="dialog" aria-modal="true" aria-label="检索资料">
    ...
    <div className="activity-source-body"><h3>{source.title}</h3>{source.knowledge_base_name ? <p>{source.knowledge_base_name}</p> : null}{source.location ? <p>{source.location}</p> : null}{error ? <p role="alert">{error}</p> : chunk ? <div className="activity-source-text">{chunk.content}</div> : <p role="status">正在读取片段…</p>}</div>
  </aside></>;
}
