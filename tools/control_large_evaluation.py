...
from tools.install_large_evaluation_launchd import LABELS, _bootout, _domain, _launchctl
...
def resume(config_path: Path) -> dict[str, Any]:
    ...
    paths = _private_plist_paths(runtime_root=_config_path(binding, "runtime_root"))
    ...
    try:
        ...
        for name in reversed(("worker", "api", "falkordb", "postgres")):
            _bootout(LABELS[name])
        raise
    checkpoint = _write_control_checkpoint(
        binding,
...
def main() -> int:
    ...
    try:
        result = pause(arguments.config) if arguments.pause else resume(arguments.config)
